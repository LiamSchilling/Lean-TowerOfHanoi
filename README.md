...
- [ ] **1** - Given `3` distinct towers, construct the walk that solves the classic configuration of `n` blocks and prove that its length is `2^n-1`.
- [ ] **2** - Prove that any solution of length less than `2^n` to the classic configuration of `n` blocks on exactly `3` towers is equal to the previous solution.
...
- [X] Map moves and walks on a lower subset of all the blocks to moves and walks on all the blocks.
